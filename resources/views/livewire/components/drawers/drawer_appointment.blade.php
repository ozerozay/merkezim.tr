...
new class extends \Livewire\Volt\Component {
    ...
    public function init(): void
    {
        try {
            ...
            $this->appointmentClientServices = $this->appointment->status == \App\AppointmentStatus::finish
                ? $this->appointment->finish_service_ids
                : $this->appointment->service_ids;
            $this->client_id = $this->appointment->client_id;

            $this->dispatch('reload-services', $this->client_id, $this->appointmentClientServices, 'yok')
                ->to('components.client.client_service_multi_dropdown');

            foreach (\App\AppointmentStatus::cases() as $status) {
                if ($status->name == 'teyitli'
                    || $status->name == 'waiting'
                    || $status->name == 'cancel'
                    || $status->name == 'merkez') {
            ...
            $this->merkezdeTeyitliStatuses = [
                ['id' => \App\AppointmentStatus::merkez->name,
                    'name' => \App\AppointmentStatus::merkez->label()],
                ['id' => \App\AppointmentStatus::teyitli->name,
                    'name' => \App\AppointmentStatus::teyitli->label()],
            ];
...
<div>
    <x-drawer wire:model="isOpen" subtitle="{{ $this->appointment->client->phone ?? '' }}"
              title="{{ $this->appointment->client->name ?? 'KAPALI' }}" class="w-full lg:w-1/3"
            ...
            @if ($appointment)
                @if ($appointment->type == \App\AppointmentType::appointment)
                    <x-accordion wire:model="group" separator class="bg-base-200">
                        @if (!(($appointment->status == \App\AppointmentStatus::finish || $appointment->status ==
                            \App\AppointmentStatus::cancel || $appointment->status == \App\AppointmentStatus::rejected)))
