...
<div>
    <x-header title="Talep" subtitle="{{ $branchName ?? '' }}" separator progress-indicator>
        <x-slot:actions>
            ...
            <x-button icon="tabler.plus" class="btn-primary"
                wire:click="$dispatch('slide-over.open', {'component': 'modals.talep.create-talep-modal'})"
                label="Talep Ekle" responsive />
        </x-slot:actions>
    </x-header>
            ...
            <x-card subtitle="{{ $talep->status->label() }} - {{ $talep->branch->name }}" separator>
                ...
                <x-slot:menu>
                    <x-button icon="tabler.settings"
                        wire:click="$dispatch('slide-over.open', {component: 'modals.talep.talep-modal', arguments: {'talep': {{ $talep->id }}}})"
                        class="btn-outline btn-sm" responsive />
                </x-slot:menu>
                ...
                <x-list-item :item="$talep">
                    ...
                    <x-slot:actions>
                        <x-button label="{{ $talep->phone }}" icon="o-phone" external
                            link="tel:0{{ $talep->phone }}" />
                    </x-slot:actions>
                </x-list-item>
                <x-list-item :item="$talep">
                    <x-slot:value>
