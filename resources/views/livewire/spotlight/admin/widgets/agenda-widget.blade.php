            ...
            <div class="flex flex-col gap-2">
                ...
                <div class="flex flex-row justify-between items-center">
                    <div class="flex items-center gap-2">
                        ...
                        <h2 class="text-base font-bold">Ajanda</h2>
                    </div>
                ...
                <div class="space-y-2">
                            ...
                            <div class="card-body p-3">
                                <div class="flex flex-col gap-2">
                                    ...
                                    <div class="flex items-start justify-between">
                                        <div class="flex items-center gap-2">
                                            <div class="badge {{ $item['type_color'] }} badge-sm">
                                                {{ $item['type_label'] }}
                                            </div>
                                            <p class="text-xs opacity-70">{{ \Carbon\Carbon::parse($item['date'])->format('d/m/Y H:i') }}</p>
                                        </div>
                                        @if($item['price'])
                                    ...
                                    <div>
                                        <div class="flex items-center gap-2">
                                            ...
                                            @if(!$item['is_owner'])
                                                <span class="text-xs opacity-50">({{ $item['user_name'] }})</span>
                                            @endif
                                        </div>
                                        @if($item['message'])
                                            <p class="text-xs opacity-70 mt-1">{{ $item['message'] }}</p>
                                        @endif
                                    </div>
                                </div>
                            </div>
                    ...
                    @empty
                        <div class="text-center py-4 text-base-content/50">
                            <p class="text-sm">Bu tarih aralığında ajanda kaydı bulunmuyor</p>
                        </div>
                    @endforelse
